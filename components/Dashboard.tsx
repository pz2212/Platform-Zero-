...
import { 
  ...
  Plus, Store, MessageSquare, Camera, Image as ImageIcon, Loader2, Send, ChevronDown, DollarSign, HelpCircle
} from 'lucide-react';
  ...
  return (
    <div className="animate-in fade-in duration-500 min-h-screen">
      <div className="mb-10">
        <div className="flex justify-between items-start mb-10">
            <div>
                ...
                <p className="text-gray-500 font-medium text-sm">Manage orders, logistics, and network directory.</p>
            </div>
      ...
      <div className="pb-20 space-y-12">
        ...
        {activeTab === 'Customers' && (
          <div className="space-y-10 animate-in fade-in duration-300 px-2">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10 px-2">
                <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-2">
                {filteredCustomers.map(customer => {
                    ...
                    return (
                        <div key={customer.id} className="p-8 bg-white border border-gray-100 rounded-[2.5rem] hover:shadow-xl transition-all group flex flex-col justify-between min-h-[400px]">
                            <div>
                                <div className="flex justify-between items-start mb-6">
                                    ...
                                    <div className={`flex items-center gap-2 px-3 py-1 rounded-full border shadow-sm ${statusCfg.color}`}>
                                        <StatusIcon size={14}/>
                                        <span className="text-[9px] font-black uppercase tracking-widest">{customer.connectionStatus || 'Connected'}</span>
                                    </div>
                                </div>
                                ...
                                <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mb-8">{customer.category}</p>
                                
                                <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100 mb-6">
                                    <div className="flex items-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5">
                                        <Store size={12} className="text-indigo-400"/> Linked Supplier
                                    </div>
                                    <p className="text-xs font-black text-gray-900 uppercase tracking-tight truncate">
                                        {customer.connectedSupplierName || 'Direct Connection'}
                                    </p>
                                </div>
                            </div>
                ...
                <div onClick={() => setIsInviteModalOpen(true)} className="border-4 border-dashed border-gray-100 rounded-[2.5rem] p-8 flex flex-col items-center justify-center text-center group hover:bg-indigo-50/30 hover:border-indigo-200 transition-all cursor-pointer min-h-[400px]">
                    ...
                    <h3 className="text-xl font-black text-gray-400 group-hover:text-gray-900 tracking-tight uppercase leading-none">Invite New Buyer</h3>
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-4">Generate direct setup link</p>
                </div>
            </div>
          </div>
        )}
