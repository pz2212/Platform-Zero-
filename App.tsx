...
const App = () => {
  ...
  // Check if we have a deep link. Deep links allow bypassing the initial landing page for specific views.
  const isDeepLink = window.location.hash.includes('item=') || window.location.hash.includes('portal=') || window.location.hash.includes('confirm=') || window.location.hash.includes('join/');

  if (!user && !isDeepLink) return (
    <>
  ...
  return (
    <Router>
      <AppLayout user={user} onLogout={handleLogout}>
        <Routes>
          <Route path="/settings" element={user ? <SettingsComponent user={user} onRefreshUser={() => setUser({...mockService.getAllUsers().find(u => u.id === user.id)!})} /> : <Navigate to="/" />} />
          <Route path="/marketplace" element={<Marketplace user={user} />} />
          <Route path="/trading-insights" element={user ? <TradingInsights user={user} /> : <Navigate to="/" />} />
          ...
          {/* Default Route for Guests or Deep Linked users */}
          {!user && (
            <>
              <Route path="/marketplace" element={<Marketplace user={null} />} />
              <Route path="*" element={<Marketplace user={null} />} />
            </>
          )}
